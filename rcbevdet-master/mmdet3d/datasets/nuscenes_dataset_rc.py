...
class NuScenesDatasetRC(Custom3DDatasetradar):
    ...
    def evaluate(self,
        ...
        if "pts_bbox" in results[0]:
            if metric == 'kitti' or (isinstance(metric, list) and 'kitti' in metric):
                return self.evaluate_kitti(results, logger=logger, show=show, out_dir=out_dir, pipeline=pipeline)
    ...
    def evaluate_kitti(self,
        ...
        # Convert predictions to KITTI format
        det_annos = self.bbox2result_kitti(results, self.CLASSES)
        
        # Get GT annotations
        gt_annos = [info['annos'] for info in self.data_infos]
        
        # Filter classes for KITTI evaluator and map to its expected names
        kitti_name_mapping = {
            'car': 'Car',
            'pedestrian': 'Pedestrian',
            'cyclist': 'Cyclist',
        }
        kitti_eval_classes = [kitti_name_mapping[c] for c in self.CLASSES if c in kitti_name_mapping]
        
        evaluation = kitti_eval(None)
        metrics = evaluation.evaluate(
            gt_annotations=gt_annos,
            dt_annotations={'pts_bbox': det_annos},
            current_class=kitti_eval_classes)
        
        # Print results like in kitti_dataset.py
        print("\nKITTI Evaluation Results:")
        for area_name, area_results in metrics.items():
            print(f"\nArea: {area_name}")
            mAP = 0
            count = 0
            for cls_name in self.CLASSES:
                # NuScenes name mapping vs KITTI
                key = f"{cls_name.capitalize()}_3d_all"
                if key in area_results:
                    val = area_results[key]
                    print(f"  {cls_name}: {val:.4f}")
                    mAP += val
                    count += 1
            if count > 0:
                print(f"  mAP: {mAP/count:.4f}")
    ...
    def bbox2result_kitti(self,
                          ...
                          pklfile_prefix=None,
                          submission_prefix=None):
        """Convert 3D detection results to kitti format."""
        ...
        for idx, pred_dicts in enumerate(mmcv.track_iter_progress(net_outputs)):
            ...
            labels = pred_dicts['labels_3d'].numpy()
            ...
            for i in range(len(box_preds_lidar)):
                box = box_preds_lidar[i]
                # Mapping LiDAR [x, y, z, l, w, h, yaw] to KITTI-style camera-like mapping for the evaluator
                # evaluator uses boxes[:, [0, 2, 3, 5, 6]] as [x, z, h, l, yaw]
                anno['name'].append(class_names[int(labels[i])])
                anno['truncated'].append(0.0)
                anno['occluded'].append(0)
                anno['alpha'].append(0.0)
                anno['bbox'].append([0, 0, 100, 100]) # Dummy
                anno['dimensions'].append([box[5], box[4], box[3]]) # h, w, l
                anno['location'].append([box[0], box[1], box[2]])
                anno['rotation_y'].append(box[6])
                anno['score'].append(scores[i])
            
            if len(anno['name']) > 0:
                anno = {k: np.stack(v) if k != 'name' else np.array(v) for k, v in anno.items()}
            else:
                anno = {
                    'name': np.array([]),
                    'truncated': np.array([]),
