...
@HOOKS.register_module()
class LidarFadingHook(Hook):
    def __init__(self, fade_epoch=100000, num_proposals=300):
        """ LidarFading Hook.
        """
        ...
        if runner.epoch == self.fade_epoch:
            ...
            for i, transform in enumerate(runner.data_loader.dataset.dataset.pipeline.transforms):
                if type(transform).__name__ == 'ObjectSample':
                    print("### LidarFading Hook ### : Pop ObjectSample from pipeline.")
                    runner.data_loader.dataset.dataset.pipeline.transforms.pop(i)
                    break
            print("### LidarFading Hook ### : Set runner.model.pts_bbox_head.num_proposals from",
                  runner.model.module.pts_bbox_head.num_proposals, "to", self.num_proposals)
            runner.model.module.pts_bbox_head.num_proposals = self.num_proposals
